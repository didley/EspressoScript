...
export const noDefaultParameter: Rule = {
    ...
    visit(node, ctx): void {
        if (ts.isParameter(node) && node.initializer !== undefined) {
            ctx.report({ ...posOf(ctx.sourceFile, node), rule: 'no-default-parameter', message: 'Default parameters are not allowed (uses `undefined` as sentinel). Wrap with a thin function instead.' })
        }
    },
}
