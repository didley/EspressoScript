...
function containsNull(typeNode) {
    if (!ts.isUnionTypeNode(typeNode))
        return false;
    return typeNode.types.some(t => ts.isLiteralTypeNode(t) && t.literal.kind === ts.SyntaxKind.NullKeyword);
}
...
function isValidAsyncReturn(typeNode) {
    ...
    if (!inner)
        return false;
    if (inner.kind === ts.SyntaxKind.VoidKeyword)
        return true;
    if (inner.kind === ts.SyntaxKind.NeverKeyword)
        return true;
...
export const requireAsyncTupleReturn = {
